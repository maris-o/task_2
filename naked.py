...
# Your NASA API key.
nasa_api_key = "dCG28InXwffUE6TsflU9u0wZRUQEC5LTqcXenIbu"
...
# Requesting info from NASA API. Construct a NASA request link with today's date and your API key
...
# Cheking if we have response from NASA API request.
if r.status_code == 200:

    # Loading API data from json.
	json_data = json.loads(r.text)

    # Crating two empty arrays
	ast_safe = []
	ast_hazardous = []

    # Printing asteroid count from requested API data
	if 'element_count' in json_data:
		ast_count = int(json_data['element_count'])
		print("Asteroid count today: " + str(ast_count))

        # Check if there were asteroids
		if ast_count > 0:
            # Loop over json data in near_earth_objects.
			for val in json_data['near_earth_objects'][request_date]:
                # Check if there is needed data about asteroid. Assign variables
				if 'name' and 'nasa_jpl_url' and 'estimated_diameter' and 'is_potentially_hazardous_asteroid' and 'close_approach_data' in val:
					tmp_ast_name = val['name']
					tmp_ast_nasa_jpl_url = val['nasa_jpl_url']
                    # From data selects the unit of measure (kilometers)
					if 'kilometers' in val['estimated_diameter']:
                        # Assign rounded variables
                    ...
                    # If close approach data found, create and assign varables from close_approach_data.
        ...
        # Print if no asteroids were today
		else:
			print("No asteroids are going to hit earth today")
    # Print hazardous and safe asteroid count
	...
	if len(ast_hazardous) > 0:
        ...
        # Loop through hazardous array list. Convert selected data index to string: asteroid speed, name, NASA url and print
